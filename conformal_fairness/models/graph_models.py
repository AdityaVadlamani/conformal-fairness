...
class BackboneGNN(torch.nn.Module):
    def __init__(
        ...
        base_layer = {
            LayerType.GCN.name: GraphConv,
            LayerType.GAT.name: SimpleGATConv,
            LayerType.GRAPHSAGE.name: SAGEConv,
        }[config.model]

        if config.layers == 1:
            l_args = (num_features, num_classes)
            l_kwargs = dict()
            if config.model == LayerType.GAT.name:
                l_args = (num_features, num_classes, config.heads)
                l_kwargs = dict(last_layer=True)
            elif config.model == LayerType.GRAPHSAGE.name:
