...
def compute_metric(metric, prediction_sets, labels, alpha=None, features=None):
    match metric:
        case ConformalMetric.SET_SIZES.name:
            return set_sizes(prediction_sets)
        case ConformalMetric.COVERAGE.name:
            return calc_coverage(prediction_sets, labels)
        case ConformalMetric.EFFICIENCY.name:
            return calc_efficiency(prediction_sets)
        case ConformalMetric.FEATURE_STRATIFIED_COVERAGE.name:
            return calc_feature_stratified_coverage(prediction_sets, features, labels)
        case ConformalMetric.SIZE_STRATIFIED_COVERAGE.name:
            return calc_size_stratified_coverage(prediction_sets, labels)
        case ConformalMetric.LABEL_STRATIFIED_COVERAGE.name:
            return calc_label_stratified_coverage(prediction_sets, labels)
        case ConformalMetric.SINGLETON_HIT_RATIO.name:
            return singleton_hit_ratio(prediction_sets, labels)
        case ConformalMetric.SIZE_STRATIFIED_COVERAGE_VIOLATION.name:
            if alpha is None:
                logging.warning(
...
def run_conformal_fairness(
    ...
    for label in labels:
        ...
        for g_i in range(datamodule.num_sensitive_groups):
            match args.fairness_metric:
                case (
                    ...
                    | FairnessMetric.EQUALIZED_ODDS.name
                ...
                case FairnessMetric.PREDICTIVE_PARITY.name:
                    try:
                        ...
                        cpf.config.fairness_metric = (
                            FairnessMetric.PREDICTIVE_PARITY.name
                        )
        ...
        elif args.fairness_metric == FairnessMetric.EQUALIZED_ODDS.name:
            print(
