...
def plot_pred_set_size(model, tau, x_test, y_test, num_groups, group_fn, mult_width, multivalid=False, num_grid=100, verbose=True, mode: Union[Literal["clf"], Literal["reg"]]="clf"):
    ...
    for i in range(len(x_test)):
        ...
        if mode == "reg":
            ...
            pred_set_size = len(np.where(pred > y_test[i])[0])
        for j in range(num_groups):
            if group_fn(x_test[i])[j]:
                group_num[j] += 1
