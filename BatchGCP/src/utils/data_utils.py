...
def load_processed_graph(dataset_path, extra_folders=""):
    features = torch.load(
        os.path.join(dataset_path, PROCESSED_DIR, extra_folders, FEATURE_FILE), weights_only=False
    )
    labels = torch.load(
        os.path.join(dataset_path, PROCESSED_DIR, extra_folders, LABEL_FILE), weights_only=False
    )
    edge_list = torch.load(
        os.path.join(dataset_path, PROCESSED_DIR, extra_folders, EDGE_FILE), weights_only=False
    )
    sens_attr = None
    if os.path.exists(
        os.path.join(dataset_path, PROCESSED_DIR, extra_folders, SENS_FILE)
        ...
        sens_attr = torch.load(
            os.path.join(dataset_path, PROCESSED_DIR, extra_folders, SENS_FILE), weights_only=False
        )
...
def prep_german(
    ...
    *,
    ...
    discard_attrs = discard_attrs or [
        "OtherLoansAtStore",
        "PurposeOfLoan",
    ]  # TODO Figure out why were doing this
...
def prep_credit(
    ...
    *,
    ...
    discard_attrs = discard_attrs or ["Single"]  # TODO Figure out why this is done
...
def prep_bail(
    ...
    *,
    ...
    csv_file = "bail.csv"  # TODO Figure out which CSV files to use
    read_nifty(
        pred_attrs=pred_attrs,
        discard_attrs=discard_attrs,
        sens_attrs=sens_attrs,
...
def schl_transform(x):
    temp = x
    e = 0.000000001
    x = ((temp>=1-e)*(temp<=15+e))*1 # No schooling (+ primar _ high school only)
...
def schl_transform_small(x):
    ...
    e = 0.000000001
    x = ((temp>=1-e)*(temp<=15+e))*1 # No schooling (+ primar _ high school only)
    x+= ((temp>=16-e) * (temp<=20+e))*2 # High School - no college  + # GED - no college + # Started college/associates
...
def prep_education_level(dataset_path,dataset_args):
    ...
    tgt_transform = schl_transform if not dataset_args.small_class else schl_transform_small
    ...
    for state in state_list:
        features, labels, sens = ACSTravelTime.df_to_numpy(data.get_data(states = [state], download=True))
        feat_list.append(torch.tensor(features, dtype=torch.float32))
        label_list.append(torch.tensor(labels, dtype=torch.int64))
        sens_list.append(torch.tensor(sens, dtype=torch.float32) - 1) # subtract 1 since groups are 1,2, ..., 9
...
def prep_acs_travel_time(dataset_path,dataset_args):
    ...
    sens_binary = dataset_args.binary_sens
    target_transform = time_breakdowns if not dataset_args.small_class else (lambda x: x > 20)
    ACSTravelTime = BasicProblem(
        features=[
            "AGEP",
            "SCHL",
...
def prep_acs_income(
    ...
    sens_binary = dataset_args.binary_sens
    target_transform = tax_breakdowns if not dataset_args.small_class else (lambda x: x > 50000)
...
def prep_enem(dataset_path,dataset_args):
    ...
    label = ['NU_NOTA_CN'] ## Labels could be: NU_NOTA_CH=human science, NU_NOTA_LC=languages&codes, NU_NOTA_MT=math, NU_NOTA_CN=natural science
    ...
    n_classes = 4 if not dataset_args.small_class else 2 # also 2 is used by the authors
    
    load_enem(dataset_path = dataset_path, 
              features=all_vars, 
...
def get_edge_list(ds_name: str, ds_dir: str, graph: dgl.DGLGraph):
    if ds_name in CUSTOM_DATASETS:
        return torch.load(os.path.join(ds_dir, ds_name, PROCESSED_DIR, EDGE_FILE), weights_only=False)
    else:
...
def load_enem(dataset_path, features, grade_attribute, n_sample, n_classes, group = 'race', multigroup=False,all_label=False):
    """
    For a given (X,y) features = X, grade_attribute = y, g is the group such that (X,y) \in g
    """
    if all_label:
        features = list(set(features) | set(['NU_NOTA_CH','NU_NOTA_LC','NU_NOTA_MT','NU_NOTA_CN']))
